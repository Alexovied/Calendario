...
function inicializarUsuario() {
    ...
    if (!userName) {
        setTimeout(() => {
            userName = prompt("¡Bienvenido al Calendario! ¿Cómo te llamas?");
            if (userName) {
                localStorage.setItem('userName', userName);
                elGreeting.textContent = `¡Hola, ${userName}!`;
                playSound('success');
            } else {
                elGreeting.textContent = `¡Hola, Invitado!`;
            }
        }, 100);
    } else {
        elGreeting.textContent = `¡Hola, ${userName}!`;
...
elThemeToggle.addEventListener('change', (e) => {
    if (e.target.checked) {
        document.body.classList.add('child-mode');
        localStorage.setItem('themeMode', 'child');
        playSound('magic');
    } else {
        document.body.classList.remove('child-mode');
        localStorage.setItem('themeMode', 'adult');
        playSound('magic');
    }
});
